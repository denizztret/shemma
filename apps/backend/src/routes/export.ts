...
export function exportRoutes(opts: ExportRoutesOpts = {}) {
  return (
    new Hono()
      ...
      .post("/api/export/miro", async (c) => {
        ...
        const rawSelection =
          Array.isArray(body.selection) && body.selection.length > 0
            ? body.selection
            : body.scope === "room"
              ? Object.keys(room.store.store).filter((id) =>
                  id.startsWith("shape:"),
                )
              : [];
        ...
        try {
          ...
          const result = await runMiroExport({
            ...
            onCommit: (r) => {
              r.dirty = true;
              scheduleSave(rv.id, r);
            },
          });
          room.dirty = true;
          scheduleSave(rv.id, room);
          ...
          if (e instanceof MiroAuthError) {
            return c.json(
              {
                ok: false,
                error: "miro-auth-failed",
                hint: "Check your token in ~/.config/shemma/config.json",
              },
              401,
            );
          }
