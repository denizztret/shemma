...
describe("CanvasClient space param — non-room methods unaffected", () => {
  it("listRooms does NOT include space in URL", async () => {
    const c = new CanvasClient({ baseUrl: "http://x", space: "s-rooms" });
    await c.listRooms();
    // listRooms hits /api/rooms — no space/room query params by design
