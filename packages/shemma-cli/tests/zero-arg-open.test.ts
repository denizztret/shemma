...
/**
 * Integration tests for `shemma` / `shemma open [<room>]` zero-arg flow (DRW-052).
 *
 * Setup: in-process test daemon на random-port-е с заведомо известным storage.
 * CLI subprocess запускается с SHEMMA_PORT=<test port>, чтобы
...
beforeAll(async () => {
  daemonStorageDir = mkdtempSync(join(tmpdir(), "shemma-zero-arg-srv-"));
...
describe("shemma zero-arg / open (DRW-052)", () => {
  test("auto-cwd: creates .shemma/canvas-dev/ in temp cwd + emits info-log", async () => {
    ...
    try {
      ...
      // Target storage = <tmpCwd>/.shemma/canvas-dev (resolved by CLI).
      ...
      const target = resolvePath(tmpCwd, ".shemma", "canvas-dev");
  ...
  test("conflict detection includes running + target paths in JSON error", async () => {
    ...
    try {
      ...
      expect(j.target).toContain(".shemma");
    } finally {
      rmSync(tmpCwd, { recursive: true, force: true });
    }
  });
  ...
  test("matching storage: no conflict, exit 0, browser suppressed by --no-browser", async () => {
    ...
    const base = mkdtempSync(join(tmpdir(), "shemma-match-base-"));
  ...
  test("--storage takes precedence over SHEMMA_STORAGE_DIR env", async () => {
    // Verify precedence side-effect: mkdir happens на --storage path, не на env path.
    const tmpCwd = mkdtempSync(join(tmpdir(), "shemma-prec-cwd-"));
    const explicitBase = mkdtempSync(join(tmpdir(), "shemma-prec-explicit-"));
    const envBase = mkdtempSync(join(tmpdir(), "shemma-prec-env-"));
    try {
  ...
  test("env SHEMMA_STORAGE_DIR used when no --storage; cwd-default suppressed", async () => {
    const tmpCwd = mkdtempSync(join(tmpdir(), "shemma-env-cwd-"));
    const envBase = mkdtempSync(join(tmpdir(), "shemma-env-base-"));
    try {
      ...
      const envSubdir = join(envBase, "canvas-dev");
      const cwdSubdir = join(tmpCwd, ".shemma");
