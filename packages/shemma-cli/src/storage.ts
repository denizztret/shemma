...
/**
 * Pure helper: вычисляет storage base для `shemma open` / zero-arg launch.
 *
 * Precedence (per DRW-052):
 *   1. explicit `--storage <path>` (already parsed → opts.explicit)
 *   2. `SHEMMA_STORAGE_DIR` env (если задано)
 *   3. auto: `<cwd>/.shemma`
 *
