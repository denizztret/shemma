...
/**
 * Implements zero-arg `shemma` и `shemma open [<room>]` (DRW-052).
 *
 * Steps:
...
export async function open(profile: Profile, opts: OpenOpts = {}) {
  ...
  if (running !== null) {
    ...
    if (runningAbs !== targetAbs) {
      ...
      console.error(
        `daemon already running on storage "${runningAbs}"; expected "${targetAbs}". Run 'shemma daemon stop' first.`,
      );
      process.exit(1);
    }
  }
  ...
  if (resolution.source === "auto-cwd") {
    console.error(`[shemma] using local storage: ${resolution.base}`);
  }
