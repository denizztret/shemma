...
describe("didraw logs", () => {
  test("--tail 5 returns last 5 lines from log file", async () => {
    tmpDir = mkdtempSync(join(tmpdir(), "didraw-logs-"));
    const logPath = join(tmpDir, ".didraw-release.log");

    // Write 20 numbered lines
    const lines = Array.from({ length: 20 }, (_, i) => `line ${i + 1}`);
    writeFileSync(logPath, lines.join("\n") + "\n");

    // Override the home dir by overriding DIDRAW_LOG_DIR (we patch via env)
  ...
  test("logs exits 2 when log file not found", async () => {
    ...
    const tmpHome = mkdtempSync(join(tmpdir(), "didraw-home-"));
    tmpDir = tmpHome;
    ...
    const script = `
      ...
      const path = join(${JSON.stringify(tmpHome)}, ".didraw-release.log");
      if (!existsSync(path)) {
...
describe("log rotation", () => {
  test("rotateLogIfNeeded creates .log.1 when file exceeds max size", async () => {
    tmpDir = mkdtempSync(join(tmpdir(), "didraw-rotate-"));
    const logPath = join(tmpDir, ".didraw-test.log");

    // Write content exceeding default 10MB; we override via DIDRAW_LOG_MAX_MB=1byte (1/1024/1024 MB)
    // Actually DIDRAW_LOG_MAX_MB=0.00001 would be tiny; let's write a 1KB file and set max to 0.0001 MB
  ...
  test("DIDRAW_LOG_MAX_MB env controls rotation threshold", async () => {
    tmpDir = mkdtempSync(join(tmpdir(), "didraw-rotate-mb-"));
    const logPath = join(tmpDir, ".didraw-test.log");
    ...
    const script = `
      process.env.DIDRAW_LOG_MAX_MB = "0.001"; // 1.024 KB threshold
      const { statSync, existsSync } = await import("node:fs");

      const maxMb = Number(process.env.DIDRAW_LOG_MAX_MB);
    ...
    const proc = Bun.spawn(["bun", "-e", script], {
      env: { ...(process.env as Record<string, string>), DIDRAW_LOG_MAX_MB: "0.001" },
      stdout: "pipe",
      stderr: "pipe",
    });
    const [code, stdout] = await Promise.all([proc.exited, new Response(proc.stdout).text()]);
