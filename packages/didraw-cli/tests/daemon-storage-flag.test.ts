...
describe("didraw daemon start --storage (argv parsing & validation)", () => {
  ...
  test("creates dir + profile subdir when --storage valid (no real daemon spawn)", async () => {
    ...
    const base = mkdtempSync(join(tmpdir(), "didraw-cli-storage-"));
    try {
      // Используем DIDRAW_PORT с занятым/неважным значением и --profile debug
      ...
      await runCli(
        ...
        // DIDRAW_PORT pinned to a port we don't expect to clash; if it does,
