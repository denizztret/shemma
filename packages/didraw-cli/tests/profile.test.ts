...
describe("runtime profile isolation", () => {
  ...
  test("storage directory differs per profile", () => {
    const devEnv = { ...process.env, DIDRAW_PROFILE: "dev" };
    devEnv.DIDRAW_STORAGE_DIR = undefined;

    const releaseEnv = { ...process.env, DIDRAW_PROFILE: "release" };
    releaseEnv.DIDRAW_STORAGE_DIR = undefined;
