...
**Цель:** добавить `@didraw/canvas-mcp` — тонкий MCP-server поверх `@didraw/client`, чтобы AI мог работать с canvas через MCP tools (typed schema, чище чем Bash через `didraw` CLI).
...
**Не блокирует MVP** — Bash через `didraw` CLI уже даёт AI полный доступ к canvas. MCP — улучшение DX (typed tools, чище transcript, меньше escape-ошибок).
...
| Файл | Назначение |
|---|---|
| `packages/canvas-mcp/package.json` | workspace package, зависит от `@didraw/client` + `@modelcontextprotocol/sdk` |
...
Per **ADR-0001** (`docs/decisions/0001-mermaid-import-location.md`) Mermaid импорт — **только browser-side** через `window.didrawImportMermaid` (требует tldraw `Editor` mounted в DOM). `CanvasClient.importMermaid` **не существует** — план в этом месте устарел.
...
DIDRAW_PORT=8787 bun packages/didraw-cli/src/index.ts daemon ensure
...
3. **Workspace-resolution** — `bun run packages/canvas-mcp/src/index.ts` должен резолвить `@didraw/client` через workspace. Если падает — добавить tsconfig path mapping или использовать relative `import` от `../didraw-client/src/index`.
...
```
feat: Phase 2.1 — canvas-mcp adapter (thin wrapper over didraw-client)
```
...
- Опционально: добавить `canvas-mcp` в `bun build --compile` pipeline (отдельный binary `canvas-mcp` или часть didraw multi-call binary).
