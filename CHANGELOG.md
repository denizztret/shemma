...
- **Style propagation (sub-project 3 после DRW-180)** — централизованное управление стилем линии (Draw/Solid), шрифтом (Draw/Sans/Mono) и размером (S/M/L/XL) для shapes. BoardPanel: defaults для нового содержимого, persistent в `room.meta.styleDefaults`. SelectionPanel: переключатель видим когда в selection ≥1 frame/schema-container; один клик атомарно меняет всех selected + всех recursive descendants + пишет sticky `meta.didrawStyleDefaults` на frame/container. Spec: `docs/superpowers/specs/2026-05-27-style-propagation-design.md` v0.2.
  - **Domain types** — `StyleDefaults` (`dash?: "draw"|"solid"`, `font?: "draw"|"sans"|"mono"`, `size?: "s"|"m"|"l"|"xl"`) + `validateStyleDefaults` + `applyStyleDefaultsResolution(chain)` + `DEFAULT_STYLE_DEFAULTS` в `@shemma/domain/style-defaults.ts`. 8 tests.
  - **Backend `/api/board/style-defaults`** — GET (raw + effective) / POST (Partial | null persist + broadcast hook). Mirror паттерна `board-layout-params`. 5 tests.
  - **Backend `/api/agent/style-apply`** — atomic sweep: `{selectedIds, styles, respectUserOwned?}`. Applicability matrix: `geo` (dash/font/size), `note`/`text` (font/size), `arrow` (dash/font/size), `schema-container` (dash + sticky meta only), `frame` (sticky meta only). Dashed/dotted preservation — shapes с current `props.dash ∈ {dashed, dotted}` пропускают dash, font/size применяются. `respectUserOwned` default `true` для AI safety (frontend SelectionPanel pass'ит `false` как explicit user action). Single `applyStoreChanges` → broadcast atomic. 5 tests.
  - **Frontend writer `applyStyleToSelection(editor, ids, styles)`** — optimistic `editor.run({ ...updateShape per target })` + POST. Recursive descendant collection через `editor.getSortedChildIdsForParent`. Sticky meta на frame/schema-container + props на всех target shapes. Frontend pass'ит `respectUserOwned: false`.
  - **Frontend derive helper `deriveUnifiedStyleState(shapes)`** — null = mixed/indeterminate, иначе одно из enum значений. Dashed/dotted shapes исключаются из dash unification (preservation invariant). Frame/schema-container исключены из font/size unification. 5 tests.
  - **UI `StylesSection`** (заменяет stub) — 3 ряда переключателей Линия/Шрифт/Размер + optional subtitle prop. Mirror tldraw native style panel visual language (`settings-btn--on` highlight). Tooltip над Линией: «применяется только к непрерывным линиям; пунктирные и точечные настраиваются через нативную панель».
  - **BoardPanel wire** — `<StylesSection>` после Компоновки. `SettingsPopover.tsx` owns state: `getStyleDefaults` на mount, `postStyleDefaults` per-key optimistic update + rollback. `BoardPanelProps` extended: `styleEffective`, `onStyleDash/Font/Size`.
  - **SelectionPanel wire** — `<StylesSection subtitle="Для выделения">` conditional через `showStyles = counts.containers > 0` (loose rule, отличается от `showContainerSections` который `containers > 0 && nodes === 0`). State derived из selected + рекурсивных descendants через `deriveUnifiedStyleState`. Handlers вызывают `applyStyleToSelection`.
  - **Bidirectional sync + container resolution** в `apps/frontend/src/canvas/style-defaults-sync.ts` (wired в `App.tsx::onMount`):
    - Initial fetch + `editor.setStyleForNextShapes(DefaultDashStyle/FontStyle/SizeStyle, value)` — native tldraw panel автоматически отражает board defaults.
    - `editor.store.listen({ source: "user", scope: "session" })` watches `instance.stylesForNextShape` changes → PATCH `room.meta.styleDefaults`. Echo-guard per-key `Map<string, {value, expiresAt}>` TTL 200ms предотвращает loop.
    - `editor.sideEffects.registerBeforeCreateHandler("shape", ...)` walks parent chain up до 64 hops, collects `meta.didrawStyleDefaults` (container-only — board-level defaults обслуживает `setStyleForNextShapes`). **Resolver fires ТОЛЬКО когда в chain есть хотя бы один container sticky** — не перетирает explicit user choice через native toolbar. Partial-only write (не fallback на native defaults).
  - **Tests:** 131 domain + 1050 backend + 47 client + 217 cli + 246 mcp + **370 frontend** (было 365 + 5 новых на `applyPartialToShape` resolver semantics) = **2061 pass / 0 fail** (+28 от baseline 2033 на main).

- **Frame & Container Direction + Layout (sub-project 1+2 после DRW-179)** — после live testing DRW-179 user сформулировал три связанные потребности: frame и schema-container должны вести себя одинаково как контейнеры компоновки, BoardPanel настройки сбивают с толку (выглядят как instant apply, на деле это defaults), и parent frame схлопывается при `⌘⇧L`/`⌘⌥⇧L` на single container внутри него. Объединённый design spec + plan: `docs/superpowers/specs/2026-05-27-frame-container-direction-layout-design.md` v0.1.1, `docs/superpowers/plans/2026-05-27-frame-container-direction-layout-plan.md` (14 tasks).
  - **Frame direction + custom** — `apps/backend/src/domain/layout.ts:readContainerDirection` теперь читает `frame.meta.didrawDirection` (TB/BT/LR/RL → ELK direction; `"custom"` → return undefined для skip Pass A). `isCustomDirection` расширен. **Frame исключается из DRW-178 auto-direction inference** через filter в `inferContainerDirections` — `meta.didrawDirection` на frame'е зарезервирован под user-set value (избегаем ambiguity). 3 tests: LR applies, custom preserves positions, frame не infer'ится (`apps/backend/tests/layout-direction-frame.test.ts`).
  - **Per-anchor layout params override** — frame и schema-container могут иметь `meta.didrawLayoutParams: Partial<LayoutParams>`. `runLayoutSubgraph` helper `paramsForAnchor(anchorId)` мерджит `{...boardParams, ...override}` через `applyLayoutParamsDefaults`. Pass A для каждого anchor использует свой `optsForContainer(id)` — board defaults остаются для Pass B / top-level. Spacing UI label "Roomy" → domain enum `loose`. `LayoutParams.spacing?: Spacing` extension в `@shemma/domain/layout-params.ts` (optional, не ломает existing callers). 3 tests: override applies для subgraph, undefined → board defaults, schema-container override аналогично (`apps/backend/tests/layout-params-override-per-frame.test.ts`).
  - **`ContainerLayoutOverride` type** — новый shared alias `Partial<LayoutParams> | null` в `@shemma/domain` (re-export через index.ts). Used both frontend writer + backend route.
  - **POST `/api/agent/layout-selection` body extended** — новое поле `layoutParamsOverride?: Record<ElementId, Partial<LayoutParams> | null>`. Backend semantics: validate через `validateLayoutParams` (atomic 400 abort при invalid), `null` → delete `meta.didrawLayoutParams`, `Partial` → write meta atomic ДО runLayout. Non-container ids → `unresolved`. Merged в single `shapeUpdated` map с directions для clean WS broadcast (Task 4). 4 tests: writes, deletes, atomic abort, non-container skip (`apps/backend/tests/routes-layout-selection-override.test.ts`).
  - **Backend directions parser принимает frame** — `parsedDirections` apply loop теперь для `frame` пишет `meta.didrawDirection` (а не `props.direction` которого у native frame нет). Frame entries больше не попадают в `unresolved` при valid direction. Это устраняет race с WS debounce (50ms) — backend атомарно применяет direction ДО runLayout. Same fix pattern как DRW-166/167 для schema-container. 3 tests (`apps/backend/tests/routes-layout-selection-frame-direction.test.ts`).
  - **Frontend writers (polymorphic + explicit ids):**
    - `setContainerDirection(editor, ids, dir)` (replaces `setSchemaContainerDirection`) — schema-container → `props.direction`, frame → `meta.didrawDirection`. Explicit ids parameter. Все callers обновлены (`SettingsPopover.tsx::onDirectionChange`, `App.tsx::onSetContainerDirection.current` context-menu). 5 tests.
    - `setContainerLayoutParams(editor, ids, partial)` — optimistic `editor.updateShape({meta:{didrawLayoutParams: partial|undefined}})` + POST с `layoutParamsOverride` map + `scope:"self"`. Reset (partial=null) — frontend пишет `undefined` (tldraw delete locally), JSON wire = `null`, backend deletes server-side. Convergent semantics. 3 tests (`apps/frontend/src/shapes/container-layout-params.test.ts`).
  - **`tidyLayout` scope heuristic** — `tidyLayout(ids, space, room, scope?)` принимает 4-й parameter; новый exported `scopeFor(ids, editor)`: single container/frame → `"self"`, иначе → `"auto"`. Hotkey handler factories `makeTidyHotkeyHandler` и `makeForceReLayoutHotkeyHandler` теперь принимают `editor` и передают scope в callback. `SettingsPopover.tsx::onLayoutAction` тоже использует `scopeFor`. `LayoutSelectionInput` extended с `scope?: "self" | "auto"`. **Это и есть fix critical bug "parent frame схлопывается при Tidy одного контейнера"** — раньше body не содержал scope, backend defaults к `"auto"` → Pass B пересчитывал frame size. 9 tests (`apps/frontend/src/canvas/tidy-layout.test.ts`).
  - **SettingsPopover useValue — polymorphic:** `counts.containers` counts schema-container ИЛИ frame; `direction` polymorphic `readDir` (props.direction для schema-container, meta.didrawDirection для frame); `showContainerSections = counts.containers > 0 && counts.nodes === 0` — conservative rule, sections видны только если ALL selected = containers (mixed → скрыто).
  - **UI components:**
    - **Новый `LayoutSettingsSection`** (preset Compact/Normal/Roomy + autoDirection toggle + midpoint even/center + Advanced link + Reset). `LayoutSettingsValue` type с `null` variants для multi-selection indeterminate state. CSS `.settings-btn--indeterminate` (45° striped pattern). 12 tests.
    - **Существующий `LayoutSection` → `LayoutActionsSection`** (Tidy / Force-unpin actions) — переименован чтобы освободить нэйм для settings. Importers обновлены.
    - **BoardPanel rebrand:** заголовок `"По умолчанию"` + neutral badge `"Для нового содержимого"` (CSS `.settings-popover__badge`). `BoardPanelAdvanced` тот же badge + helper text `"Эти значения работают как defaults для всего room"`. **SelectionPanel** specifically **БЕЗ badge** — отсутствие = visual fingerprint instant apply (vs BoardPanel = defaults). CSS `.settings-popover__hint` + `.settings-popover__title`. 4 tests.
    - **SelectionPanel wiring** — extended props: `showContainerSections`, `layoutSettings`, `onPreset`, `onAutoDirection`, `onMidpoint`, `onAdvanced`, `onReset`, `showReset`. SettingsPopover wires через `setContainerLayoutParams`. `buildPartial` helper accumulates current `meta.didrawLayoutParams` чтобы изменение ONE field не стирало остальные. `showReset` selector — visible если any selected has `meta.didrawLayoutParams`. 5 tests.
  - **Re-layout scope invariant suite** — `apps/backend/tests/layout-selection-scope-invariant.test.ts` (3 cases): `scope:"self"` preserves frame size, `scope:"auto"` может update legitimately, multi-leaf `scope:"auto"` frame size update OK. Task 0 investigative pass confirmed backend `containerScope === "self"` уже корректно skip'ает Pass B + writeback фильтрует через `affectedIds` — реальный bug чисто во frontend (`tidy-layout.ts:41` не передавал `scope`).
  - **Tests:** 1031 backend (was 1024) + 9 new layout/route tests + 3 regression invariant tests = **1040 backend pass**. Frontend 320 → **359 pass** (+ 39 new across container-actions / container-layout-params / tidy-layout / sections / panels). **Все 1399+ tests pass, 0 fail**.
  - **Live verification** через chrome-devtools MCP подтвердил: bug-fix scope:"self" preserves frame (`1396×2048` unchanged); scope:"auto" reproduces bug (`300×264`); `meta.didrawLayoutParams` writes + reset работают; BoardPanel показывает header "По умолчанию" + badge "Для нового содержимого" + Direction/Компоновка/Стили sections + "Все 16 параметров →"; SelectionPanel (контейнер selected) показывает Direction + Компоновка (новая) + actions Упорядочить/Принудительно + Pin + footer "1 контейнер", **без badge**, Авто-направление с indeterminate striped state.

- **DRW-179** — Settings popover: Option+click открывает контекстную панель поверх canvas через tldraw `components.InFrontOfTheCanvas` slot. Три panel'а в одном popover'е:
  - **SelectionPanel** (multi-selection, single Frame, single container): Direction (TB/LR/BT/RL/Custom) → POST `/api/agent/layout-selection` с `direction`; Layout actions — `Tidy ⌘⇧L` и `Force re-layout ⌘⇧⌥L` (новый optional `forceUnpin: boolean` body-флаг для `POST /api/agent/layout-selection`); Pin size/position bulk-toggle через `editor.updateShape({meta})` + WS sync (как role-picker).
  - **NodePanel** (одна shape с `meta.didrawId` или legacy `meta.didrawName`): Pin size/position + inline Role picker (reuses `ALL_ROLES` из `@shemma/domain`).
  - **BoardPanel** (пустой canvas): default Direction + Spacing presets `Compact / Normal / Roomy` (маппинг 4 LayoutParams: nodePadding/containerPadding/edgeSpacing/edgeNodeSpacing) + Auto-direction toggle + midpoint mode (even/center) + Styles stub + "All 16 params →" drill-down в inline replacement same popover.
  - **Storage:** новое `room.meta.layoutParams: Partial<LayoutParams>` управляется `GET+POST /api/board/layout-params?space&room` (space/room в query — соблюдает `spaceMiddleware`). `runLayout` теперь читает `roomMeta.layoutParams` на всех 5 call-sites (schema.ts:847/1016/1365, layout-selection.ts:250, domain.ts:283); экспонирует `appliedParams: LayoutParams` в return для observability.
  - **Trigger:** Alt+pointerdown в capture phase на window; filter по `editor.getViewportScreenBounds()`; close на Escape / click outside / camera + selection change.
  - **A11y v1:** focus-trap на mount, Tab/Shift+Tab cycling, `role="dialog" + aria-label`, `role="radiogroup" + aria-checked` для Direction/Layout/Spacing, `role="switch"` для Pin/Auto-direction, `aria-describedby` + sr-only hints для presets.
  - **Files:** `apps/frontend/src/settings/` (12 файлов) + `apps/backend/src/routes/board-layout-params.ts` + backend types `LayoutHint.forceUnpin` (`apps/backend/src/domain/types.ts`).

- **DRW-178** — Routed-arrow followup на native tldraw `kind:"elbow"` + smart layout pipeline. После двух откатанных попыток (attempt-1 с runtime libavoid + custom routed-arrow shape, attempt-codex с конверсией native→routed) выбран принципиально новый подход: остаёмся на native tldraw arrows (реактивны на shape move из коробки), всю интеллектуальность переносим в layout-стадию.
  - **Phase 1** — `kind:"elbow"` дефолт в `apps/backend/src/domain/compile.ts:makeArrowShape` и `apps/backend/src/routes/schema.ts:makeArrowShapeLocal`. Heuristic label measure `packages/shemma-domain/src/label-metrics.ts:measureLabelHeuristic` (avg-char-width формула без DOM). `apps/backend/src/domain/layout.ts:computeLabelDerivedSpacing` (в `buildElkGraph` + `runLayoutSubgraph`) гарантирует `org.eclipse.elk.layered.spacing.nodeNodeBetweenLayers ≥ maxLabelWidth + 2·margin`.
  - **Phase 2** — `apps/backend/src/domain/midpoints.ts:computeElbowMidpoints` распределяет `elbowMidPoint = (i+1)/(N+1)`. Группировка fan-in `(target, targetSide)` + fan-out `(source, sourceSide)` с приоритетом fan-in. Подключён в `apps/backend/src/routes/_anchors.ts:runAndBroadcastAnchors` после `computeAnchors`. `normalizeDirection` больше не collapse'ит BT→TB / RL→LR — 4-way directions выживают end-to-end через `MERMAID_DIR_TO_ELK` (TB→DOWN, BT→UP, LR→RIGHT, RL→LEFT). `apps/backend/src/domain/directions.ts:determineContainerDirection` — 4-way side-count heuristic.
  - **Phase 2.6** — auto-direction inference с parallel-lanes detection. `inferContainerDirection`: если ≥2 ребёнка имеют external edges все на ОДНОЙ стороне → flip перпендикулярно parent (на A→C/B→D через TB-parent даёт LR-lanes). `apps/backend/src/domain/layout.ts:inferContainerDirections` walk'ит containers pre-layout, пишет `meta.didrawDirection`. Differentiator `meta.didrawDirectionInherited` (true для subgraph'ов без explicit `direction X` в mermaid) — inference срабатывает только на inherited. `readContainerDirection` приоритет: inferred ⊕ inherited > props.direction. Heuristic intentionally conservative: при отсутствии parallel-lanes match → inherit parent (избегаем BT/RL регрессий на цепях типа Оркестрация в DRW-173).
  - **Phase 3** — `packages/shemma-domain/src/layout-params.ts:LayoutParams` (16 параметров: nodeMin*, edgeLabel*, midpointDistribution, autoDirectionEnabled и др). `applyLayoutParamsDefaults` + `validateLayoutParams`. Threaded через `runLayout(store, hint, index, paramsPartial?)`, `computeLabelDerivedSpacing`, `computeElbowMidpoints` (`fixed-0.5` short-circuits), `determineContainerDirection` (`autoDirectionEnabled:false` short-circuits). `apps/backend/src/routes/layout.ts` body принимает `params` с validation → 400 на bad input. `layout-determinism.test.ts` — одна topology с разными стартовыми позициями = identical final.
  - **Phase 4** — `apps/backend/src/domain/empty-space.ts`: pure helpers `findEmptySlot` (grid-scan center-bias) + `computeExpansion` (TB/BT/LR/RL expansion rules). `apps/backend/src/routes/smart-insert.ts:POST /api/smart-insert` принимает `{containerId, size, biasDirection?, padding?}`, возвращает `{position, expansion: null | {dw, dh}}`. Read-only — caller сам создаёт shape по returned position. Arrow shapes исключаются из occupants.
...
- **DRW-174** — После DRW-171 текст помещался в shape'ах, но frame и schema-container'ы не пересчитывались под выросшие children → дети визуально вылезали за нижнюю границу родителя, sibling subgraphs не разъезжались. Корень: backend `runLayout.shapeBounds` читал только `props.h`, игнорируя `props.growY` (tldraw geo/note хранят добавочную высоту для текста в `growY`); ELK Pass A видел старый estimate 220×80, frame.props.h ставился под estimate, а frontend autosize дополнительно ставил growY-only локально (через WS как user-source change, но без trigger'а re-layout). Fix two-prong:
  - **Backend `apps/backend/src/domain/layout.ts:shapeBounds`** теперь возвращает `h = props.h + props.growY` для `geo`/`note` — ELK Pass A учитывает effective height, frame Pass A `newH` покрывает реальные visual bounds детей.
  - **Backend `POST /api/schema/:frameId/measured-bounds`** (новый endpoint в `apps/backend/src/routes/schema.ts`): принимает `{bounds: Record<shapeId, {w?, h?}>, clientOpId?}`, проверяет что каждый shape — descendant frame'а, применяет normalized bounds (для geo/note также сбрасывает `growY=0` так что effective h не двоится), затем re-runs layout с `affectedIds = {frameId}` → frame-expand внутри runLayout dispatches Pass A на детей + Pass B на siblings; WS broadcast'ит measure batch (source=user) + layout batch (source=ai).
  - **Frontend `apps/frontend/src/canvas/auto-size.ts:triggerAutoSize`**: после локального `updateShape` (immediate visual feedback из DRW-171) теперь ещё walk'ит parent chain до schema-frame ancestor; для shapes внутри v2 schema-frame собирает effective bounds `(w, h+growY)` и POST'ит per-frame на новый endpoint. Shapes без schema-frame ancestor (legacy v1, free-floating) идут только локальным путём — backward compat preserved.
