...
- **Тёмная тема (DRW-217).** Board-level переключатель 🌓 в шапке доски (цикл светлая → тёмная → системная; иконка отражает режим): tldraw canvas/UI переключается через `colorScheme`/`updateUserPreferences` без reload, режим `system` следует OS-теме live (matchMedia). Выбор хранится per-browser в `localStorage shemma:theme` (в canvas state/room store НЕ пишется) и применяется до первого рендера (без flash). Все кастомные поверхности shemma темизированы через CSS-переменные `--shemma-*` (`:root` / `html[data-shemma-theme=dark]`): design-tokens переведены на `var()` (26 компонентов — панели, кнопки, popover'ы, модалки, prompts, gallery, spaces — подхватили автоматически), `settings/styles.css` переведён с hardcoded hex на переменные, точечные inline-цвета (Gallery/RoomCard/RoomListRow/NewRoomForm) заменены на токены. SchemaContainerShape уже был theme-aware (`useColorMode`) — проверен в dark. Gallery/Spaces (страницы без editor) темизируются тем же контроллером.
- **Видимая кнопка панели настроек ⚙ в шапке доски (DRW-206).** Панель настроек (DRW-179) открывалась только по Option+RightClick / ⌘⇧P — недискавериваемо. Теперь постоянная кнопка ⚙ в верхней зоне (перед кнопкой Prompts 💬, сдвинутой left:320→356) открывает/закрывает панель в актуальном контексте (board-панель без выделения, selection/node при выделении — общий `toggle()` с ⌘⇧P); повторный клик и Escape закрывают; Option+RightClick без регрессий; стиль по design-tokens (паритет с Prompts), `aria-pressed` отражает состояние.
